...
2. Run `python3 tests/imav_bt/test_trees_smoke.py` — the mission must still tick to
   SUCCESS.
...
`tests/imav_bt/test_trees_smoke.py` picks new missions up automatically once they are in
`CHAIN`, but add the module to the list in
`test_every_mission_builder_ticks_to_completion`:

```bash
python3 tests/imav_bt/test_trees_smoke.py
