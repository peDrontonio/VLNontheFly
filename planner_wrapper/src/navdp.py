...
class PlannerNode(Node):
    ...
    def __init__(self):
        ...
        self.planner = NavDP_Agent(self.intrinsic,
                                image_size=224,
                                memory_size=8,
                                predict_size=24,
                                temporal_depth=16,
                                ...
                                device='cuda:0')
        self.planner.reset(1, self.stop_threshold)
    ...
    def planning(self):
        try:
            ...
            image = image.reshape((batch_size, image.shape[0], image.shape[1], 3))
            # 🔹 planner_output
            execute_trajectory, all_trajectory, all_values, trajectory_mask = self.planner.step_pointgoal(goal, image, depth)
            
            trajectory_points_camera = execute_trajectory
            all_trajectories_camera = all_trajectory
            all_values_camera = all_values
            ...
            current_all_trajectories = all_trajectories_world 
            current_all_values = all_values_camera[0] 
            ...
            image_for_vis = image[0]
            
            try:
                vis_image = self.vis_manager.visualize_trajectory(
                    image_for_vis, depth_for_vis[:,:,None], self.intrinsic,
                    current_trajectory,
                    robot_pose=x0,
