...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    node_->declare_parameter("fsm/goal_gate_y_max", 1.0e4);
    node_->declare_parameter("fsm/goal_gate_z_min", -1.0e4);
    node_->declare_parameter("fsm/goal_gate_z_max", 1.0e4);
    node_->declare_parameter("fsm/keepout_x", std::vector<double>{});
    node_->declare_parameter("fsm/keepout_y", std::vector<double>{});
    ...
    node_->get_parameter("fsm/goal_gate_y_max", gate_y_max_);
    node_->get_parameter("fsm/goal_gate_z_min", gate_z_min_);
    node_->get_parameter("fsm/goal_gate_z_max", gate_z_max_);
    node_->get_parameter("fsm/keepout_x", keepout_x_);
    node_->get_parameter("fsm/keepout_y", keepout_y_);
    node_->get_parameter("fsm/keepout_radius", keepout_r_);

    if (keepout_x_.size() != keepout_r_.size() || keepout_y_.size() != keepout_r_.size())
    {
      size_t n = std::min({keepout_x_.size(), keepout_y_.size(), keepout_r_.size()});
      RCLCPP_ERROR(node_->get_logger(),
                   "fsm/keepout_{x,y,radius} sizes differ (%zu/%zu/%zu) — using first %zu keep-out(s)",
                   keepout_x_.size(), keepout_y_.size(), keepout_r_.size(), n);
      keepout_x_.resize(n);
      keepout_y_.resize(n);
      keepout_r_.resize(n);
    }
    if (goal_gate_enable_)
      RCLCPP_INFO(node_->get_logger(),
                  "Goal gate ON: box x[%.1f,%.1f] y[%.1f,%.1f] z[%.1f,%.1f], %zu keep-out(s)",
                  gate_x_min_, gate_x_max_, gate_y_min_, gate_y_max_, gate_z_min_, gate_z_max_,
                  keepout_r_.size());
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    else
    {
      ...
      if (target_type_ == TARGET_TYPE::MANUAL_TARGET)
        pubGoalStatus("failed:plan");
    }
  }

  bool EGOReplanFSM::goalAllowed(const Eigen::Vector3d &wp, std::string &reason)
  {
    if (!goal_gate_enable_)
      return true;

    if (wp(0) < gate_x_min_ || wp(0) > gate_x_max_ ||
        wp(1) < gate_y_min_ || wp(1) > gate_y_max_ ||
        wp(2) < gate_z_min_ || wp(2) > gate_z_max_)
    {
      reason = "zone";
      return false;
    }
    ...
    {
      double dx = wp(0) - keepout_x_[i];
      double dy = wp(1) - keepout_y_[i];
      if (std::sqrt(dx * dx + dy * dy) < keepout_r_[i])
      {
        reason = "keepout";
        return false;
      }
  ...
  void EGOReplanFSM::pubGoalStatus(const std::string &status)
  {
  ...
  void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    ...
    RCLCPP_INFO(node_->get_logger(), "Goal (%.2f, %.2f, %.2f) accepted by safety bubble",
                end_wp(0), end_wp(1), end_wp(2));
    pubGoalStatus("accepted:safety_bubble");
