...
double max_yaw_rate_ = 3.1415926; // max yaw rate [rad/s], configurable via param (default PI = original behavior)
...
std::pair<double, double> calculate_yaw(double t_cur, Eigen::Vector3d &pos, rclcpp::Time &time_now, rclcpp::Time &time_last)
{
  ...
  const double YAW_DOT_MAX_PER_SEC = max_yaw_rate_; // "yaw speed" param, limits how fast the drone re-orients
...
int main(int argc, char **argv)
{
  ...
  // "yaw speed": max yaw rate in rad/s. Lower it to make the drone turn slower/smoother.
  node->declare_parameter("traj_server/max_yaw_rate", 3.1415926);
  node->get_parameter("traj_server/max_yaw_rate", max_yaw_rate_);
  RCLCPP_INFO(node->get_logger(), "[Traj server]: max_yaw_rate = %.3f rad/s (%.1f deg/s)",
              max_yaw_rate_, max_yaw_rate_ * 180.0 / 3.1415926);
